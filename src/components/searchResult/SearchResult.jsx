...
function SearchResults() {
  ...
  return (
   ...
   <Header className="mb-5 "/>
    <div className="flex flex-wrap justify-center gap-8 mx-8 mt-8">
      {searchedMovies.map((movie) => (
        <div key={movie.id}>
          <Link to={`/details/${movie.id}`}>
            <div className="rounded-xl w-64 h-96 hover:scale-110 transform transition-transform duration-300">
              <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="rounded-sm" />
